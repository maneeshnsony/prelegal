...
- Auth: `backend/app/security.py` (bcrypt password hashing, JWT creation/decoding with a `SESSION_SECRET_KEY`) and `backend/app/deps.py` (`get_current_user_id` FastAPI dependency reading the `Authorization: Bearer` header) replace the old fake email-only login. `backend/app/routers/auth.py` now exposes `POST /api/auth/signup` and `POST /api/auth/login`, both password-verified, returning a JWT. Google OAuth is confirmed still not implemented — `auth.py` only handles email/password. `signup` catches the `IntegrityError` from `users.email`'s unique constraint and returns 409 rather than 500 when two concurrent requests race for the same email (fixed post-merge, covered by `test_concurrent_signup_same_email_never_returns_500` in `backend/tests/test_auth.py`).
