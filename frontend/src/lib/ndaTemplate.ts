...
// Standard Terms body of the Common Paper Mutual Non-Disclosure Agreement
// (https://commonpaper.com/standards/mutual-nda/1.0/), CC BY 4.0.
// The <span class="coverpage_link">...</span> markers indicate cover-page
// values that get substituted with the user's form input below.
...
function fillCoverPageLinks(text: string, data: NdaFormData): string {
  return text
    .replaceAll('<span class="coverpage_link">Purpose</span>', data.purpose || "[Purpose]")
    .replaceAll(
      '<span class="coverpage_link">Effective Date</span>',
      data.effectiveDate || "[Effective Date]"
    )
    .replaceAll('<span class="coverpage_link">MNDA Term</span>', data.mndaTerm || "[MNDA Term]")
    .replaceAll(
      '<span class="coverpage_link">Term of Confidentiality</span>',
      data.termOfConfidentiality || "[Term of Confidentiality]"
    )
    .replaceAll(
...
export interface NdaParagraph {
  number: number;
  title: string;
  body: string;
}

export function getStandardTermsParagraphs(data: NdaFormData): NdaParagraph[] {
  const filled = fillCoverPageLinks(STANDARD_TERMS_TEMPLATE, data);
  return filled
    .split("\n")
    .filter(Boolean)
    .map((line) => {
      ...
      if (!match) {
        return { number: 0, title: "", body: line };
      }
...
export function renderPlainTextDocument(data: NdaFormData): string {
  ...
  lines.push("----------");
  lines.push(`Party A: ${data.partyAName || "[Party A Name]"}`);
  lines.push(`Party B: ${data.partyBName || "[Party B Name]"}`);
  lines.push(`Effective Date: ${data.effectiveDate || "[Effective Date]"}`);
  lines.push(`Purpose: ${data.purpose || "[Purpose]"}`);
  lines.push(`MNDA Term: ${data.mndaTerm || "[MNDA Term]"}`);
  lines.push(`Term of Confidentiality: ${data.termOfConfidentiality || "[Term of Confidentiality]"}`);
  lines.push(`Governing Law: ${data.governingLaw || "[Governing Law]"}`);
  lines.push(`Jurisdiction: ${data.jurisdiction || "[Jurisdiction]"}`);
  lines.push("");
